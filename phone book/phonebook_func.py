...
def addToList(self):
    ...
    var_fullname = ("{} {}".format(var_fname, var_lname)) # combine normalized names
    print("var_fullname: {}".format(var_fullname))#silent, only for development, user does not see 
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
    if not "@" or not "." in var_email: #will use this soon
        print("Incorrect email format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #Enforce user to use both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            chkName = count
            if chkName == 0:
                print("chkName: {}".format(chkName))#for development, remove before go live
